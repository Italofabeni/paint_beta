...
canvas.addEventListener("mousedown", (event) => {   const { clientX, clientY } = event   
...
canvas.addEventListener("mousemove", (event) => {   const { clientX, clientY } = event
    if(isPainting){ 
        if(activeTool == "brush"){
            draw(clientX, clientY)
        }
...
canvas.addEventListener("mouseup", (event) => { const { clientX, clientY } = event   
   isPainting = false
})
